...
class ColorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create color" do
    assert_difference('Color.count') do
      post colors_url, params: { color: { color_id: @color.color_id, description: @color.description, rgb: @color.rgb } }
    end
  ...
  test "should update color" do
    patch color_url(@color), params: { color: { color_id: @color.color_id, description: @color.description, rgb: @color.rgb } }
